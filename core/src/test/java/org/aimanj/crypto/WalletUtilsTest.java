...
import static org.aimanj.crypto.SampleKeys.CREDENTIALS;
import static org.aimanj.crypto.SampleKeys.KEY_PAIR;
import static org.aimanj.crypto.SampleKeys.PASSWORD;
import static org.aimanj.crypto.WalletUtils.isValidAddress;
import static org.aimanj.crypto.WalletUtils.isValidPrivateKey;
...
public class WalletUtilsTest {
    ...
    @Ignore  // enable if users need to work with wallet
    @Test
    public void testLoadCredentialsMyManWallet() throws Exception {
        Credentials credentials = WalletUtils.loadCredentials(
                PASSWORD,
                new File(WalletUtilsTest.class.getResource(
                        "/keyfiles/"
    ...
    public void testGetDefaultKeyDirectory() {
        assertTrue(WalletUtils.getDefaultKeyDirectory("Mac OS X")
                .endsWith(String.format("%sLibrary%sMatrix", File.separator, File.separator)));
        assertTrue(WalletUtils.getDefaultKeyDirectory("Windows")
                .endsWith(String.format("%sMatrix", File.separator)));
        assertTrue(WalletUtils.getDefaultKeyDirectory("Linux")
                .endsWith(String.format("%s.Matrix", File.separator)));
    }
