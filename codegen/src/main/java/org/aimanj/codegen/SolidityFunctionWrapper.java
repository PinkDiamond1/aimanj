...
import org.aimanj.utils.Collection;
import org.aimanj.utils.Strings;
import org.aimanj.utils.Version;
...
public class SolidityFunctionWrapper extends Generator {
    ...
    private static final String AIMANJ = "aimanj";
    ...
    private static final String CODEGEN_WARNING = "<p>Auto generated code.\n"
            ...
            + "<p>Please use the "
            + "<a href=\"https://docs.aimanj.io/command_line.html\">aimanj command line tools</a>,\n"
    ...
    private void addAddressesSupport(TypeSpec.Builder classBuilder,
        ...
        if (addresses != null) {
            ...
            // See org.aimanj.tx.Contract#getStaticDeployedAddress(String)
    ...
    private TypeSpec.Builder createClassBuilder(String className, String binary) {
        ...
        String javadoc = CODEGEN_WARNING + getAiManjVersion();
    ...
    private String getAiManjVersion() {
        String version;

        try {
            // This only works if run as part of the aimanj command line tools which contains
            // a version.properties file
            version = Version.getVersion();
        } catch (IOException | NullPointerException e) {
            version = Version.DEFAULT;
        }
        return "\n<p>Generated with aimanj version " + version + ".\n";
    }
    ...
    private static MethodSpec buildDeployWithParams(
        ...
        if (isPayable && !withGasProvider) {
            methodBuilder.addStatement(
                    ...
                    className, AIMANJ, authName, GAS_PRICE, GAS_LIMIT, BINARY, INITIAL_VALUE);
            methodBuilder.addAnnotation(Deprecated.class);
        } else if (isPayable && withGasProvider) {
            methodBuilder.addStatement(
                    "return deployRemoteCall("
                            + "$L.class, $L, $L, $L, $L, encodedConstructor, $L)",
                    className, AIMANJ, authName, CONTRACT_GAS_PROVIDER, BINARY, INITIAL_VALUE);
        } else if (!isPayable && !withGasProvider) {
            methodBuilder.addStatement(
                    "return deployRemoteCall($L.class, $L, $L, $L, $L, $L, encodedConstructor)",
                    className, AIMANJ, authName, GAS_PRICE, GAS_LIMIT, BINARY);
            methodBuilder.addAnnotation(Deprecated.class);
        } else {
            methodBuilder.addStatement(
                    "return deployRemoteCall($L.class, $L, $L, $L, $L, encodedConstructor)",
                    className, AIMANJ, authName, CONTRACT_GAS_PROVIDER, BINARY);
        }
        ...
        if (withGasProvider) {
            toReturn.addParameter(ContractGasProvider.class, CONTRACT_GAS_PROVIDER)
                    .addStatement("return new $L($L, $L, $L, $L)", className,
                            CONTRACT_ADDRESS, AIMANJ, authName, CONTRACT_GAS_PROVIDER);
        } else {
            toReturn.addParameter(BigInteger.class, GAS_PRICE)
                    .addParameter(BigInteger.class, GAS_LIMIT)
                    .addStatement("return new $L($L, $L, $L, $L, $L)", className,
                            CONTRACT_ADDRESS, AIMANJ, authName, GAS_PRICE, GAS_LIMIT)
                    .addAnnotation(Deprecated.class);
        }
    ...
    TypeSpec buildEventResponseObject(
            String className,
            List<org.aimanj.codegen.SolidityFunctionWrapper.NamedTypeName> indexedParameters,
            List<org.aimanj.codegen.SolidityFunctionWrapper.NamedTypeName> nonIndexedParameters) {

        TypeSpec.Builder builder = TypeSpec.classBuilder(className)
                .addModifiers(Modifier.PUBLIC, Modifier.STATIC);

        builder.addField(LOG, "log", Modifier.PUBLIC);
        for (org.aimanj.codegen.SolidityFunctionWrapper.NamedTypeName
    ...
    MethodSpec buildEventFlowableFunction(
        ...
        MethodSpec.Builder flowableMethodBuilder =
                MethodSpec.methodBuilder(generatedFunctionName)
                        .addModifiers(Modifier.PUBLIC)
                        .addParameter(ManFilter.class, FILTER)
        ...
        flowableMethodBuilder
                .addStatement("return aimanj.manLogFlowable(filter).map($L)", converter);
    ...
    MethodSpec buildDefaultEventFlowableFunction(
        ...
        flowableMethodBuilder.addStatement("$1T filter = new $1T($2L, $3L, "
                + "getContractAddress())", ManFilter.class, START_BLOCK, END_BLOCK)
    ...
    private static Class<?> getStaticArrayTypeReferenceClass(String type) {
        try {
            return Class.forName("org.aimanj.abi.datatypes.generated.StaticArray" + type);
