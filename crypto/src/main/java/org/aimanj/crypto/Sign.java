...
import static org.aimanj.utils.Assertions.verifyPrecondition;
...
public class Sign {
    ...
    static final String MESSAGE_PREFIX = "\u0019Matrix Signed Message:\n";

    static byte[] getMatrixMessagePrefix(int messageLength) {
        return MESSAGE_PREFIX.concat(String.valueOf(messageLength)).getBytes();
    }

    static byte[] getMatrixMessageHash(byte[] message) {
        byte[] prefix = getMatrixMessagePrefix(message.length);
    ...
    public static SignatureData signPrefixedMessage(byte[] message, ECKeyPair keyPair) {
        return signMessage(getMatrixMessageHash(message), keyPair, false);
    }
    ...
    /**
     * Given an arbitrary piece of text and an Matrix message signature encoded in bytes,
    ...
    /**
     * Given an arbitrary message and an Matrix message signature encoded in bytes,
    ...
    public static BigInteger signedPrefixedMessageToKey(
            byte[] message, SignatureData signatureData) throws SignatureException {
        return signedMessageHashToKey(getMatrixMessageHash(message), signatureData);
    }
